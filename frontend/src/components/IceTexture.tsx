...
const ICE_TEXTURE_HEIGHT = 1000;

function IceTexturePattern() {
  return (
    <svg
...
function IceTexture({
  ...
  const [textureCount, setTextureCount] =
    useState(1);
  ...
  useEffect(() => {
    ...
    const resizeObserver =
      new ResizeObserver(updateTextureCount);
  ...
  return (
    ...
    >
      <div
        className="ice-texture-patterns"
        style={{
          height: `${
            textureCount *
            ICE_TEXTURE_HEIGHT
      ...
      >
        {Array.from(
          ...
          (_, index) => (
            <div
              className="ice-texture-instance"
              key={index}
            >
              <IceTexturePattern />
            </div>
